...
@section('title', __('trans.school_mangement_system') . ' | ' . __('trans.list_receipt'))
                                    ...
                                    <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                                        ...
                                        @foreach($receipt_students as $index => $receipt_student)
                                            ...
                                            <td>{{$receipt_student->description}}</td>
                                                <td>
                                                    <a href="{{route('student-receipt.edit',$receipt_student->id)}}" class="btn btn-info btn-sm" role="button" aria-pressed="true"><i class="fa fa-edit"></i></a>
                                        ...
                                        @include('dashboard.pages.students.receipt.delete')
                                        @endforeach
                                    </table>
                                </div>
                            </div>
