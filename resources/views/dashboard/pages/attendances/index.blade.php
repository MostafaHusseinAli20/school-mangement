...
@section('title', __('trans.school_mangement_system') . ' | ' . __('trans.list_attendances'))
...
@section('content')
    <div>
        <h4 style="font-family: 'Cairo', sans-serif; padding-bottom: 5px;">{{ trans('trans.list_attendances') }}</h4>
    </div>
    <!-- row -->
    ...
    <h5 style="font-family: 'Cairo', sans-serif;color: red"> {{ trans('trans.day_date') }} : {{ date('Y-m-d') }}</h5>
    <form method="post" action="{{ route('attendances.store') }}">
        ...
        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
            ...
            <thead>
                <tr>
                    ...
                    <th class="alert-success">{{ trans('trans.Email') }}</th>
                    <th class="alert-success">{{ trans('trans.gender') }}</th>
                    <th class="alert-success">{{ trans('trans.grade') }}</th>
                    <th class="alert-success">{{ trans('trans.classes') }}</th>
                    <th class="alert-success">{{ trans('trans.section') }}</th>
                    <th class="alert-success">{{ trans('trans.processes') }}</th>
                </tr>
            </thead>
            <tbody>
        ...
        <P>
            <button class="btn btn-success" type="submit">{{ trans('trans.submit') }}</button>
        </P>
    </form><br>
    <!-- row closed -->
@endsection
